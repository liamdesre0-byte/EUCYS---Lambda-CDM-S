...
| Quantity (dataset it belongs to) | ΛCDM (baseline) | ΛCDM+S |
| :--- | :---: | :---: |
| training ln L (joint, n=1495) | 335.7 | **439.9** |
| training AIC (same joint, k=2 vs 4) | −667.4 | **−871.7** |
| training BIC (same joint) | −656.8 | **−850.5** |
| held-out DES-SN Y5 χ² (1820 SNe) | **7429.7** | 7579.9 |
| held-out Δχ² (ΛCDM+S − ΛCDM) | — | **+150.2 (ΛCDM preferred)** |
| ln Z (Bayesian evidence) | not yet computed | not yet computed (nested sampling pending) |
| H₀ [km/s/Mpc] | 71.64 (best fit) | 74.17 [73.53, 74.60] |
| t_crit [Gyr] | — | 8.79 [6.47, 13.55] |
| k [Gyr⁻¹] | — | 0.176 [0.115, 0.531] |

**Reading this honestly:** ΛCDM+S fits the training joint substantially
better (its extra flexibility reconciles the SH0ES and Planck pulls), but
**generalizes worse** — flat ΛCDM wins the held-out DES comparison by
Δχ² ≈ 150. This is the classic overfitting signature that information
criteria are meant to catch, observed here directly on independent data.
The scientifically interesting shift: the data place the transition
midpoint at t_crit ≈ 8.8 Gyr — near the actual onset of cosmic
acceleration — not at the previously assumed 15.8 Gyr in the future, and
prefer a slower transition (k ≈ 0.18 Gyr⁻¹) than the prior.
Both DES χ² values are inflated (χ²/dof ≈ 4) because the held-out
likelihood is diagonal with no zero-point profiling; the *difference*
between models is the meaningful number.
